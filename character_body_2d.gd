...
func _ready() -> void:
	...
	print("world_ref type: ", world_ref)  # ← Add this line
	print("world_ref script: ", world_ref.get_script())  # ← Add this line
...
func _setup_ui() -> void:
	...
	dice_label.text = "%s %s — Press SPACE to roll the dice!" % [GameData.player_emoticon, GameData.player_name]
...
func _move_next_tile() -> void:
	...
	if not world_ref.is_walkable(next_grid):
		if move_direction == Vector2(0, -TILE_SIZE):
			blocked_direction = "up"
		elif move_direction == Vector2(0, TILE_SIZE):
			blocked_direction = "down"
		elif move_direction == Vector2(-TILE_SIZE, 0):
